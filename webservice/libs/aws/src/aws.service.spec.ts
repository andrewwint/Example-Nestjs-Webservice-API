...
describe('AwsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [AwsService, { provide: AwsService, useFactory: mockAwsService }]
    }).compile();
  ...
  describe('describeInstance', () => {
    it('Make API call to AWS', async () => {
      ...
      jest.spyOn(service, 'describeInstance').mockImplementation(() => awsInstance);

      const result = await service.describeInstance('production-eu-west-2-hys');
      expect(service.describeInstance).toHaveBeenCalled();
      expect(service.describeInstance).toHaveBeenCalledWith('production-eu-west-2-hys');
  ...
  describe('getRegionByFilterKey', () => {
    it('Finds AWS Region by well formed Tag.Key.StackName', async () => {
      expect(realservice.getRegionByFilterKey('production-us-east-1-hillspet')).toStrictEqual(
        'us-east-1'
      );

      expect(realservice.getRegionByFilterKey('staging-us-east-2-solstice')).toStrictEqual(
        'us-east-2'
      );

      expect(realservice.getRegionByFilterKey('production-me-south-1-adnoc')).toStrictEqual(
        'me-south-1'
      );

      expect(realservice.getRegionByFilterKey('production-us-east-1-ip')).toStrictEqual(
        'us-east-1'
      );
    });

    it('Finds AWS Region by malformed Tag.Key.StackName defaulting to us-east-1', async () => {
      expect(realservice.getRegionByFilterKey('production-us-east-1')).toStrictEqual('us-east-1');

      expect(
        realservice.getRegionByFilterKey('production-us-east-2-client-client-something_hello')
      ).toStrictEqual('us-east-2');

      expect(realservice.getRegionByFilterKey('us-east-2-solstice')).toStrictEqual('us-east-1');

      expect(
        realservice.getRegionByFilterKey('client-client-something_hello_us-east-2-solstice')
      ).toStrictEqual('us-east-1');

      expect(realservice.getRegionByFilterKey('hello-us-east-two-solstice')).toStrictEqual(
        'us-east-1'
      );
    });
  });
